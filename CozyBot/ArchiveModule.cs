...
namespace CozyBot
{
  public class ArchiveModule : IGuildModule
  {
    ...
    public string ModuleXmlName { get => _moduleXmlName; }
    ...
    public SocketGuild Guild { get => _guild; }
        ...
        if (acp.Timer)
        {
          if (_timersDict.TryGetValue(acp.Id, out Timer timer))
          {
            ...
            if ((acp.Interval * 60000L) != (long)(timer.Interval))
            {
              ...
              Task.Run(async () => { await DumpChannelById(acp.Id).ConfigureAwait(false); });
    ...
    {
      ulong id;
      if (channelEl.Attribute("id") == null || !UInt64.TryParse(channelEl.Attribute("id").Value, out id))
        return null;

      DateTime last = new DateTime(0);
      if (channelEl.Attribute("last") != null &&
          !DateTime.TryParse(channelEl.Attribute("last").Value, out last))
      ...
      if (channelEl.Attribute("timer") != null &&
          !Boolean.TryParse(channelEl.Attribute("timer").Value, out timer))
      ...
      if (channelEl.Attribute("interval") != null)
      {
        ...
        else if (interval < _minimumInterval)
        {
      ...
      if (channelEl.Attribute("image") != null &&
          !Boolean.TryParse(channelEl.Attribute("image").Value, out image))
      ...
      if (channelEl.Attribute("silent") != null &&
          !Boolean.TryParse(channelEl.Attribute("silent").Value, out silent))
        channelEl.Attribute("silent").Value = silent.ToString();
      else
        channelEl.Add(new XAttribute("silent", silent.ToString()));

      string filepath;
      if (!String.IsNullOrEmpty(channelEl.Value))
        filepath = Directory.Exists(channelEl.Value) ? channelEl.Value : Path.Combine(_workingPath, "archive", id.ToString());
      else
      {
    ...
    private void GenerateDmpCommands(List<ulong> dmpPermissionList)
    {
      ...
      Rule dumpRule = RuleGenerator.HasRoleByIds(allPerms) & RuleGenerator.PrefixatedCommand(_prefix, "dump");

      IBotCommand dmpCmd = new BotCommand($"{StringID}-dumpcmd",
                                          dumpRule,
                                          DumpCommand);
    ...
    protected virtual void GenerateCfgCommands(List<ulong> perms)
    {
      ...
      IBotCommand configCmd = new BotCommand($"{StringID}-configcmd",
                                             cmdRule,
                                             ConfigCommand);
    ...
    protected virtual async Task DumpCommand(SocketMessage msg)
    {
      ...
      if (words.Length == 1 && words[0] == "x!dump")
        await DumpChannelById(msg.Channel.Id).ConfigureAwait(false);
      else if (words[0] == "x!dump" && msg.MentionedChannels.Count > 0)
    ...
    private async Task DumpChannel(ArchiveChannelProperties acp)
    {
      ...
      XElement currentChanneEl = _moduleConfigEl.Elements("channel")
                                 .FirstOrDefault(ch => ch.Attribute("id").Value == acp.Id.ToString());
      if (currentChanneEl == null)
        return;

      SocketTextChannel channel = _guild.GetChannel(acp.Id) as SocketTextChannel;
      ...
      var messages = await FetchMessages(channel).ConfigureAwait(false);
      ...
      while (!boundary)
      {
        messages = await FetchMessages(channel, boundaryMsg, Discord.Direction.Before).ConfigureAwait(false);
        totalList.AddRange(messages);
      ...
      foreach (var message in totalList)
      {
        ...
        if (acp.Image && message.Attachments.Count > 0)
          imageList.Add(message);
      }

      int lines = await WriteMessages(totalList, acp).ConfigureAwait(false);
      ...
      if (acp.Image)
      {
        int images = await WriteImages(imageList, acp).ConfigureAwait(false);
    ...
    private async Task<int> WriteMessages(List<Discord.IMessage> list, ArchiveChannelProperties acp)
    {
      ...
      {
        string tempStr = log.Remove(0, log.IndexOf('-') + 1);
        tempStr = tempStr.Remove(tempStr.IndexOf('.'));
        if (Int32.TryParse(tempStr, out int temp) && temp > lastNumber)
        {
          lastNumber = temp;
          lastLog = log;
        }
      ...
      if (File.Exists(lastLog) && (new FileInfo(lastLog)).Length > 8388608)
      {
        lastNumber++;
        lastLog = Path.Combine(filepath, $"{acp.Id}-{lastNumber}.log");
      }
      ...
      await using var sw = File.AppendText(lastLog);
      foreach (var message in list)
        if (message.Timestamp.UtcDateTime >= acp.Last)
        {
          await sw.WriteLineAsync($"[{message.Timestamp}] : {message.Author.Username} : {message.Content}").ConfigureAwait(false);
          lines++;
        }
    ...
    private async Task<int> WriteImages(List<Discord.IMessage> list, ArchiveChannelProperties acp)
    {
      ...
      int cores = Environment.ProcessorCount < 3 ? 1 : Environment.ProcessorCount - 1;
      int count = list.Count / cores;
      int remainder = list.Count % cores;
      List<List<IMessage>> splitList = new List<List<IMessage>>();
      int curCount;
      int j = 0;
      for (int i = 0; i < cores; i++)
      {
        curCount = (i < remainder) ? count + 1 : count;
        ...
        tasklist.Add(Task.Run(async () =>
        {
          foreach (var message in messagesPerCore)
          {
            ...
            foreach (var att in message.Attachments)
            {
              ...
              var response = await hc.GetAsync(att.Url).ConfigureAwait(false);
    ...
    protected virtual async Task ConfigCommand(SocketMessage msg)
    {
      ...
      foreach (var channelEl in _moduleConfigEl.Elements("channel"))
        if (channelEl.Attribute("id") != null && UInt64.TryParse(channelEl.Attribute("id").Value, out ulong id))
      ...
      foreach (var mentionedChannel in msg.MentionedChannels)
      {
        ...
        foreach (var word in words)
        {
          ...
          if (Int32.TryParse(word, out int interval) && interval >= _minimumInterval)

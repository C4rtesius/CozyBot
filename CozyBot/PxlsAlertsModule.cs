...
namespace CozyBot
{
  public class PxlsAlertsModule : IGuildModule, IDisposable
  {
    ...
    static string _templateRegexPattern = @"template=(?<uri>https?[\:\w%\.\/]+(?:%2F|\/)(?<file>\w+))";
    ...
    {
      add    { if (value != null) _configChanged += value; }
      remove { if (value != null) _configChanged -= value; }
    }
...
#if DEBUG
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] _moduleConfig.Alerts is NULL");
      }
      else
      {
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] _moduleConfig.Alerts is NOT NULL");
      }
      {
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Interval value: {_moduleConfig.Alerts.Interval}");
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] ChannelId value: {_moduleConfig.Alerts.ChannelId}");
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] TemplateIds Any(): {_moduleConfig.Alerts.TemplateIds.Any()}");
#endif
      }
    ...
    private PxlsAlertsModuleConfig LoadConfig(string filePath)
    {
      ...
      catch (Exception ex)
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] Config loading failed.",
    ...
    private void CreateDefaultConfig()
    {
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] Default config creation failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
    ...
    private async Task StatsCmd(SocketMessage msg)
    {
      ...
      {
#if DEBUG
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Entered StatsCmd.");
#endif
        ...
        if (!(msg.Author is SocketGuildUser author))
          return;
        var r = author.Roles.Select(r => r.Id);
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                          $"[EXCEPT][{_stringID.ToUpper()}] Stats cmd failed.",
                          $"Exception caught: {ex.Message}",
                          $"Stack trace: {ex.StackTrace}"));
        throw;
      }
    ...
    {
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Entered StatsUsrCmd.");
#endif
      var q = _moduleConfig.Stats.Users.FirstOrDefault(u => u.UserId == msg.Author.Id);
      if (q == null)
      ...
      using (var fs = new FileStream(statsJsonPath, FileMode.Open, FileAccess.ReadWrite))
      {
        ...
        {
          Console.WriteLine(String.Join(Environment.NewLine,
                                        $"[EXCEPT][{_stringID.ToUpper()}][StatsUsrCmd] Stats deserialisation failed.",
                                        $"Exception caught: {ex.Message}",
                                        $"Stack trace: {ex.StackTrace}"));
          throw;
        }
    ...
    private async Task StatsGlobalCmd(SocketMessage msg)
    {
      ...
      using (var fs = new FileStream(statsJsonPath, FileMode.Open, FileAccess.ReadWrite))
      {
        ...
        {
          Console.WriteLine(String.Join(Environment.NewLine,
                                        $"[EXCEPT][{_stringID.ToUpper()}][StatsGlobalCmd] Stats deserialisation failed.",
                                        $"Exception caught: {ex.Message}",
                                        $"Stack trace: {ex.StackTrace}"));
          throw;
        }
    ...
    {
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Entered StatsCanvasCmd.");
#endif
      if (_moduleConfig.Stats.LastDownload < DateTime.UtcNow - TimeSpan.FromMinutes(15.0d))
        await DownloadStatsJson().ConfigureAwait(false);
      ...
      using (var fs = new FileStream(statsJsonPath, FileMode.Open, FileAccess.ReadWrite))
      {
        ...
        {
          Console.WriteLine(String.Join(Environment.NewLine,
                                        $"[EXCEPT][{_stringID.ToUpper()}][StatsCanvasCmd] Stats deserialisation failed.",
                                        $"Exception caught: {ex.Message}",
                                        $"Stack trace: {ex.StackTrace}"));
          throw;
        }
    ...
    private async Task DownloadStatsJson()
    {
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] Stats download failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
    ...
    {
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Entered StatsCfgCmd.");
#endif
      var words = msg.Content.Split(' ');
    ...
    private async Task AlertCmd(SocketMessage msg)
    {
      ...
      if (!(msg.Author is SocketGuildUser author))
        return;
      if (!author.Roles.Select(r => r.Id).Contains(_pxlsOfficerId))
        return;
    ...
    {
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Entered AlertOnCmd.");
#endif
      var words = msg.Content.Split(' ');
      var tName = words[2];
      ...
      if (_moduleConfig.Alerts.TemplateIds.Contains(id))
        return;
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Got id {id}");
#endif

      var ch = _guild.GetChannel(_moduleConfig.Alerts.ChannelId) as ISocketMessageChannel;
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Got channel {ch.Id}");
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Got interval {_moduleConfig.Alerts.Interval}");
#endif

      await GetStatus(q.First().Url, ch, false).ConfigureAwait(false);
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] GetStatusFinished.");
#endif
      ...
      await SaveConfig().ConfigureAwait(false);
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Config Saved.");
#endif
    }
    ...
    private async void AlertTimerElapsed(object o, ElapsedEventArgs e)
    {
      ...
      if (!(_guild.GetChannel(_moduleConfig.Alerts.ChannelId) is ISocketMessageChannel ch))
        return;
    ...
    private async Task GetStatus(string inputUri, ISocketMessageChannel sc, bool cmd)
    {
...
#if DEBUG
      {
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Invalid uri :{templateMatch.Groups["uri"].Value}");
        return;
      }
#else
        return;
...
#if DEBUG
      {
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Invalid output :{output}");
        return;
      }
#else
        return;
...
#if DEBUG
      {
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Invalid offsetx :{offsetxString}");
        return;
      }
#else
        return;
...
#if DEBUG
      {
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Invalid offsety :{offsetxString}");
        return;
      }
#else
        return;
...
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Checks passed.");
#endif
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                          $"[EXCEPT][{_stringID.ToUpper()}] Template download failed.",
                          $"Exception caught: {ex.Message}",
                          $"Stack trace: {ex.StackTrace}"));
        throw;
      }
...
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Template downloaded.");
#endif
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] Template loading failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] Canvas data download failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Canvas data downloaded.");
#endif
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] Detemplatized image save failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
...
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Image detemplatized.");
#endif
...
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Wrongmap formed.");
#endif
...
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Wrongmap image processed and formed.");
#endif
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] WrongMap image save failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
...
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Wrongmap image saved.");
#endif
      try
      {
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] WrongMap image or message send failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Message and image sent.");
#endif
      try
      {
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] Temporary files deletion failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Temp files deleted.");
#endif
    }
    ...
    private async Task HelpCmd(SocketMessage msg)
    {
      ...
      if (msg.Author is SocketGuildUser user)
      {
        ...
        {
          Console.WriteLine(String.Join(Environment.NewLine,
                                        $"[EXCEPT][{_stringID.ToUpper()}] Command call deletion failed {msg.Channel.Name}.",
                                        $"Exception caught: {ex.Message}",
                                        $"Stack trace: {ex.StackTrace}"));
          throw;
        }
        ...
        var efb = new EmbedFieldBuilder
        {
          ...
          Value = String.Join(Environment.NewLine,
                              @$"`{_prefix}status [посилання на шаблон|назва шаблону]` - стан виконання шаблону за посиланням",
                              @$"`{_prefix}palette list` - список палітр",
                              @$"`{_prefix}palette add [назва палітри] [файл палітри]` - додати палітру",
                              @$"`{_prefix}palette get [назва палітри] ?[text|file]` - отримати палітру у вигляді тексту або файлу",
                              @$"`{_prefix}palette del [назва палітри]` - видалити палітру",
                              @$"`{_prefix}template list` - список шаблонів",
                              @$"`{_prefix}template add [назва шаблону]` [посилання на шаблон] - додати шаблон",
                              @$"`{_prefix}template del [назва шаблону]` - видалити шаблон",
                              @$"`{_prefix}template get [назва шаблону]` - отримати шаблон та метадані",
                              @$"`{_prefix}template detempl [пікча|посилання] ?[назва палітри]` - отримати зображення з шаблона",
                              @$"`{_prefix}template make [пікча|посилання] ?dotted ?[назва палітри]` - отримати шаблон з зображення",
                              @$"`{_prefix}stats ?[m|me]` - ваша статистика",
                              @$"`{_prefix}stats [g|gl|glob|global]` - статистика за весь час",
                              @$"`{_prefix}stats [c|can|canv|canvas]` - статистика за цей канвас",
                              " **Якщо вас нема в статистиці - звертайтесь до Піксельних Офіцерів.**",
    ...
    private async Task DetemplByAttachmentCmd(SocketMessage msg)
    {
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] Canvas data fetch failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
    ...
    private async Task<string> DetemplHelper(SocketMessage msg, string file, IEnumerable<Rgba32> pd)
    {
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}][DetemplHelper] Image load failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}][DetemplHelper] Image save failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
...
#if DEBUG
        {
          Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Invalid uri :{url}");
          return String.Empty;
        }
#else
          return String.Empty;
    ...
    private (string, int) CreateDottedTemplate(string srcFile)
    {
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}][CreateDottedTemplate] Save failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
    ...
    private async Task TemplateGetCmd(SocketMessage msg)
    {
      ...
      string output = String.Join(Environment.NewLine,
                                  ...
                                  $@"`Name : {t.Name}`",
                                  $@"`URL : `{t.Url}",
                                  $@"`Image URL : `{t.SourceUrl}",
                                  $@"`Added by : {t.AddedBy}`",
                                  $@"`Palette ID : {"Not implemented"}`");
      if (!(msg.Author is SocketGuildUser author))
        return;
    ...
    private async Task TemplateAddCmd(SocketMessage msg)
    {
      ...
      var url = words[3];
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Template Add Cmd URL :{url}");
#endif
      if (!Uri.IsWellFormedUriString(url, UriKind.RelativeOrAbsolute))
      {
        msg.Channel.SendMessageAsync("Bad template URL.");
#if DEBUG
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] URL BAD.");
#endif
        return;
      }
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] URL OK.");
#endif
      if (_moduleConfig.Templates.Any(t => t.Name == name))
      {
    ...
    private async Task SendNetFilePalette(SocketMessage msg, Palette p)
    {
      ...
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}][SendNetFilePalette] Operation failed.",
                                      $"Exception caught: {ex.Message}",
                                      $"Stack trace: {ex.StackTrace}"));
        throw;
      }
    ...
    private async Task PaletteAddCmd(SocketMessage msg)
    {
      ...
      if (msg.Attachments.Count == 1)
      {
        ...
        {
          string name = words[2];
          if (_moduleConfig.Palettes.Any(p => p.Name == name))
          {
        ...
        {
          Console.WriteLine(String.Join(Environment.NewLine,
                                        $"[EXCEPT][{_stringID.ToUpper()}][PaletteAddCmd] Operation failed.",
                                        $"Exception caught: {ex.Message}",
                                        $"Stack trace: {ex.StackTrace}"));
          throw;
        }
    ...
    private async Task SaveConfig()
    {
      ...
      {
#if DEBUG
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] SaveConfig Triggered.");
        Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] _moduleConfig.Stats is null: {_moduleConfig.Stats == null}");
        if (_moduleConfig.Stats != null)
        {
          Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] _moduleConfig.ChannelId: {_moduleConfig.Stats.ChannelId}");
          Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] _moduleConfig.Interval: {_moduleConfig.Stats.Interval}");
        }
#endif
      ...
      {
        Console.WriteLine($"[EXCEPT][{_stringID.ToUpper()}] Catched :{ex.Message}");
        Console.WriteLine($"[EXCEPT][{_stringID.ToUpper()}] Stack   :{ex.StackTrace}");
        throw;
      }
    }
    ...
    {
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] Entered StatusCmd.");
#endif
      string[] words = msg.Content.Split(" ");
      string inputUri = words[1];
#if DEBUG
      Console.WriteLine($"[DEBUG][{_stringID.ToUpper()}] words[1] :{words[1]}");
#endif
      await GetStatus(inputUri, msg.Channel, true).ConfigureAwait(false);
    }
    ...
    static async Task<CanvasData> GetCanvasData(string infoUri, string boardUri)
    {
      ...
      response = await _hc.GetAsync(boardUri).ConfigureAwait(false);
      response.EnsureSuccessStatusCode();
      var canvasData = await response.Content.ReadAsByteArrayAsync().ConfigureAwait(false);

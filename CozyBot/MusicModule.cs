  ...
  public class MusicModule : IGuildModule
  {
    ...
    private async Task PlayCmd(SocketMessage msg)
    {
      try
      {
        ...
        await using var aus = audioClient.CreatePCMStream(AudioApplication.Music);
        ...
        finally { await aus.FlushAsync().ConfigureAwait(false); }
      ...
      catch (Exception ex)
      {
        Console.WriteLine(String.Join(Environment.NewLine,
                                      $"[EXCEPT][{_stringID.ToUpper()}] Music Playback failed.",
